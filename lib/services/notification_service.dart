...
// Update to "matched" when both users like each other
  ...
  Future<void> adminApproveMatch(
      String adminNotificationId, String feedback) async {
    try {
      ...
      List<String> relatedNotifications =
          List<String>.from(adminData['relatedNotifications']);
  ...
  Future<void> sendAcceptanceNotification(
      String recipientUserId, String senderName, String organType) async {
    try {
      String notificationId = _firestore.collection('notifications').doc().id;
